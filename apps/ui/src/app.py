...
from ui.correlation_visuals import render_correlation_visuals
...
def main() -> None:
    ...
    _render_diagnostics_sidebar(runtime_settings, schema, filter_values)
...
_PRIMARY_DIM_NAMES = {"lob_description"}
...
def _render_filters(schema: SemanticSchema, values: FilterValues) -> Dict[str, Any]:
    ...
    with st.container(border=True):
        ...
        with primary_cols[4]:
            if primary_dims:
                primary_dim = primary_dims[0]
                dv = dim_value_map[primary_dim.name]
                selected = _render_dimension_widget(
                    dim_name=primary_dim.name,
                    dim_description=primary_dim.description,
                    dv=dv,
                )
                if selected is not None:
                    dimension_selections[primary_dim.name] = selected
            else:
                st.caption("")
...
def _build_research_payload(selections: Dict[str, Any], raw_question: str) -> Dict[str, Any]:
    ...
    end_month = _coerce_int(time_selections.get(principal_name)) if principal_name else None
...
def _render_pattern_tab(
    ...
    render_recommended_actions(recommended_actions)
...
def _infer_sql_stage(filename: str) -> str:
...
def _collect_sql_records(sql_files: List[Path]) -> List[Dict[str, Any]]:
    ...
    for sql_file in sql_files:
        try:
            ...
            records.append({
                ...
                "stage": _infer_sql_stage(sql_file.name),
                "lines": 0,
...
def _render_sql_queries(output: Dict[str, Any]) -> None:
    ...
    if sql_files:
        _render_sql_catalog(_collect_sql_records(sql_files))
        return
