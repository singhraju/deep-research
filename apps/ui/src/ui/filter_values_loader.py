...
def load_all_filter_values(schema: SemanticSchema) -> FilterValues:
    ...
    logger.info(
        "filter-loader: initial load for view='%s' | dimensions=%d time_dimensions=%d snowflake_enabled=%s",
        schema.view_name,
    ...
    time_ranges = tuple(_load_one_time_dimension(tdim) for tdim in schema.time_dimensions)
