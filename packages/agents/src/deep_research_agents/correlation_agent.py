...
class PeriodWindowSchema(BaseModel):
    ...
    baseline_start_time: Optional[int] = None
    baseline_end_time: Optional[int] = None
...
DEFAULT_SEMANTIC_MODEL_PATH = (
    Path(__file__).resolve().parents[4] / "configs" / "correlation_pattern" / "coc_ecap_claims_custom_am67979.yaml"
)
DEFAULT_STOP_RULES = {
    "max_depth": 4,
    "top_k_per_level": 5,
...
def resolve_period_window(intent: IntentPayload) -> PeriodWindow:
    ...
    if not comparison_start_value:
        raise AgentConfigurationError(
            ...
            "Provide a YYYYMM value (e.g., 202601) in your analysis_mode_parameters.period.current_period.start_time."
        )
    comparison_start = int(comparison_start_value)
    ...
    if not comparison_end_value:
        raise AgentConfigurationError(
            ...
            "Provide a YYYYMM value (e.g., 202603) in your analysis_mode_parameters.period.current_period.end_time."
        )
    comparison_end = int(comparison_end_value)
    ...
    baseline_start = int(
        ...
        or shift_yyyymm_by_year(comparison_start, -1)
    )
    baseline_end = int(
        previous_period.get("end_time")
        or period_dict.get("baseline_end_time")
        or shift_yyyymm_by_year(comparison_end, -1)
    )
    ...
    return {
        ...
        "baseline_months": expand_yyyymm_window(baseline_start, baseline_end),
        "comparison_months": expand_yyyymm_window(comparison_start, comparison_end),
    }
...
def period_case_sql(time_expr: str, period_window: PeriodWindow) -> str:
    return (
        "CASE "
        f"WHEN {time_expr} BETWEEN {period_window['baseline_start_time']} AND {period_window['baseline_end_time']} THEN 'baseline' "
        f"WHEN {time_expr} BETWEEN {period_window['start_time']} AND {period_window['end_time']} THEN 'comparison' "
        "ELSE NULL END"
    )
...
def period_filter_sql(time_expr: str, period_window: PeriodWindow) -> str:
    return (
        f"(({time_expr} BETWEEN {period_window['baseline_start_time']} AND {period_window['baseline_end_time']}) "
        f"OR ({time_expr} BETWEEN {period_window['start_time']} AND {period_window['end_time']}))"
    )
...
def build_period_extract_query(
    ...
    if period_label == "baseline":
        time_window_clause = f"{time_expr} BETWEEN {period_window['baseline_start_time']} AND {period_window['baseline_end_time']}"
    else:
        time_window_clause = f"{time_expr} BETWEEN {period_window['start_time']} AND {period_window['end_time']}"
...
def execute_correlation(state: GraphState) -> Dict[str, Any]:
    ...
    period_window = resolve_period_window(intent)
    ...
    for level in range(1, max_depth + 1):
        ...
        path_prefix = "_".join(folder_token_for_dimension(node["dimension"]) for node in drill_path)
        for dimension_name in available_dimensions:
            ...
            full_folder_name = folder_name if not path_prefix else f"{path_prefix}_{folder_name}"
            ...
            query_path = queries_dir / f"{full_folder_name}.sql"
            ...
            if save_parquet:
                aggregate_dir = ensure_dir(aggregates_dir / full_folder_name)
