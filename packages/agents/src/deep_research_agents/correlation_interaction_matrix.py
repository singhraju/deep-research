...
def _dimension_label_from_catalog(
    catalog: SemanticCatalog, dimension_name: str
) -> str:
    fields = catalog.get("fields_by_name", {}).get(dimension_name, [])
    first = fields[0] if fields else None
    return derive_dimension_label(first, dimension_name)


def _metric_label_from_catalog(
    catalog: SemanticCatalog, metric_name: str
) -> str:
    metric = catalog.get("metrics_by_name", {}).get(metric_name)
    if metric:
    ...
    return humanize_metric_name(metric_name)


def build_interaction_matrix_schema(
    catalog: SemanticCatalog,
    operational_dimensions: Sequence[str],
    clinical_dimensions: Sequence[str],
    explainer_metric_names: Sequence[str],
) -> Dict[str, Any]:
    """Emit a UI hint describing the dimension/metric vocabulary of this run.

    The UI consumes this to build labels and pick column formatters instead of
    hardcoding old-schema keys — that way any config selected in
    ``configs/{env}.ini`` renders without code changes.
    """

    return {
        "operational_dimensions": [
            {"name": name, "label": _dimension_label_from_catalog(catalog, name)}
        ...
        "explainer_metrics": [
            {
                "name": name,
                "label": _metric_label_from_catalog(catalog, name),
                "role": classify_metric(name),
            }
            for name in explainer_metric_names
        ],
    }
...
def execute_interaction_matrix(
    *,
    ...
    matrix_schema = build_interaction_matrix_schema(
        catalog,
        operational_dimensions,
        clinical_dimensions,
        interaction_explainers,
    )
