...
#     semantic_cfg = _build_semantic_card_config(semantic, analysis_mode_name)
...
# def _build_semantic_card_config(semantic: Mapping[str, Any], analysis_mode_name: str) -> dict[str, Any]:
...
#         "dimension_roles": _infer_dimension_roles(dim_catalog),
...
#     role_keywords = {
#         "geography": ("state", "market", "region", "county", "zip", "geo", "service area"),
...
#         "dimensions": dims,
#         "context_dimensions": dict(ctx.get("dominant_drill_context") or {}),
#         "dimension_sources": dimension_sources,
#         "canonical_dimensions": canonical,
#         "metrics": metrics,
#         "driver_type": driver,
#         "flags": flags,
#         "filters": _filters_from_dims(raw_dims),
...
#                 "dimensions": dims,
#                 "context_dimensions": {},
#                 "dimension_sources": {dim: "drill_path_distribution"},
#                 "canonical_dimensions": canonical,
#                 "metrics": metrics,
...
#     geos = [value for value, _ in dims_by_role.get("geography", Counter()).most_common(5)]
...
#     fallback = {
#         "geography": ("state", "market", "region", "county", "zip"),
...
# def _filters_from_dims(dims: Mapping[str, Any]) -> list[dict[str, Any]]:
...
# Special clinical split rule: keep inpatient oncology/chemotherapy, maternity/C-section, and rare catastrophic Med/Surg/transplant/ECMO/trach outlier patterns separate when they appear as material drivers, because their business actions differ.
# 7. Aim to cover all analytical groups, but narrative clarity takes priority over forced compression. It is better to return 8 focused patterns than 7 patterns that hide a distinct business lever.
# 8. source_group_ids define the business scope of the pattern. source_card_ids are representative evidence, not the full scope. Do not shrink estimated_delta, priority_entities, key_driver_codes, or the business narrative to only the selected source_card_ids when the pattern intentionally uses multiple source_group_ids.
# 9. Preserve traceability using group_ids and card_ids, but keep traceability out of the business narrative.
# 10. Use rounded numbers:
...
# 11. Be direct. If a pattern needs validation before action, say so.
# 12. Avoid vague recommendations such as \"review trends\" or \"monitor closely.\" Tie every pattern to a concrete business lever.
...
#     def __init__(self, agent_name: str = PATTERN_AGENT_NAME, **kwargs: Any) -> None:
#         super().__init__(agent_name=agent_name, state_class=PatternGraphState, **kwargs)
...
#         try:
#             try:
#                 structured_llm = llm.with_structured_output(PatternSynthesisSchema, include_raw=True)
#                 response = structured_llm.invoke(messages)
#                 raw_response = response.get("raw") if isinstance(response, dict) else response
#                 parsed = response.get("parsed") if isinstance(response, dict) else response
#             except TypeError:
#                 structured_llm = llm.with_structured_output(PatternSynthesisSchema)
...
#             states[str(value)] += 1
#         for value in top_dimensions.get("geography", []) or []:
#             states[str(value)] += 1
...
#     state_counter = Counter(dims_by_role.get("geography", Counter()))
