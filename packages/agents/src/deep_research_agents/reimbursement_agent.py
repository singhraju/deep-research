...
class ReimbursementState(TypedDict, total=False):
    ...
    search_keywords: List[str]  # Generated from DRGs for policy search (list of keywords)
    title_anchors: List[str]  # Title-substring anchors for post-fetch topical filter; empty = skip filter
    search_context_status: str  # "ok" | "insufficient" — set by search_policies_node
    ...
    llm_nano: Any  # LLM client for policy processing (GPT-5.4-nano for speed)
    llm_mini: Any  # LLM client used only on extract retry (GPT-5.4-mini, stronger instruction-following)
...
Extract ONLY the minimal code/identifier WITHOUT any descriptive prefix text.
Strip labels like "CPT ", "Modifier ", "RC ", "Code ", hyphens, or parenthetical
descriptions. If you see "Modifier D - Diagnostic", extract just "D". If you see
"CPT 99291 (critical care)", extract just "99291". Keep each array entry ≤8 words.
Use empty arrays (not null) when nothing applies.

IMPORTANT: Distinguish between CODES and MODIFIERS:
- CODES (target_codes/related_codes): Procedure/diagnosis identifiers — always
  have digits (99291, G0378, 469, A0427). Even HCPCS has letter + 4 DIGITS.
- MODIFIERS (required_modifiers/excluded_modifiers): Claim-line qualifiers —
  typically 1-3 alphanumeric chars (25, 59, XU, GM, QL, QM, D, E, H).
If you see 1-2 uppercase letters (GM, QL, QN) in an ambulance policy, those are
MODIFIERS, not codes. Put them in required_modifiers or excluded_modifiers.
...
- target_codes: Extract ONLY procedure/diagnosis code identifiers. Must be either
  (a) 3-5 digits with optional letter (CPT/DRG: 99291, 469, 99284T), or
  (b) letter + 4 digits (HCPCS: G0378, A0427), or
  (c) letter + 2+ digits with optional decimal (ICD-10: O30.001, C50.9).
  Accept ranges (e.g., ["99210-99220"], ["A0426-A0428"]).
  Do NOT put 1-2 letter codes here — those are modifiers (see below).
  Examples: ✓ ["99291", "G0378", "469", "99210-99220"] ✗ ["GM", "QL", "CPT 99291"]
- related_codes: Same rules as target_codes. Other procedure/diagnosis codes
  referenced in the condition (e.g., ["99281", "99285", "G0378"]).
  Do NOT put modifiers (GM, QL, QM, QN, etc.) here — use required_modifiers/excluded_modifiers.
- required_modifiers / excluded_modifiers: Extract ONLY the 1-3 character
  modifier code. Strip any prefix text like "Modifier ", "modifier ", leading
  hyphens, or descriptions. Examples:
    ✓ Correct: ["25"], ["59", "XU"], ["D", "E"], ["GM", "QL"]
    ✗ Wrong: ["Modifier 25"], ["modifier D"], ["-H"], ["Modifier GM"]
  If the policy says "Modifier D - Diagnostic site", extract just "D".
  If the policy says "-H (Hospital)", extract just "H".
- revenue_codes: Extract ONLY the 3-5 digit code, preserving X wildcards and
  ranges. Strip any prefix like "RC ", "Revenue Code ", etc. Examples:
    ✓ Correct: ["045X", "0762", "068X", "0450-0459"]
    ✗ Wrong: ["RC 0762", "Revenue Code 045X", "0450 (emergency)"]
...
2. target_codes, related_codes, required_modifiers, excluded_modifiers, and
   revenue_codes must contain ONLY the minimal code identifier without any
   prefix labels or descriptions. Do NOT copy:
   - Prompt headers: "DRG Codes:", "CPT/HCPCS Codes:", "Pattern:"
   - Code type prefixes: "CPT ", "HCPCS ", "Modifier ", "RC ", "Code "
   - Leading symbols: "-" (unless it's part of a range like "99210-99220")
   - Descriptions: "Modifier D - Diagnostic", "CPT 99291 (critical care)"
   Extract ONLY: "99291", "D", "0762", "99210-99220"
...
Return ONLY valid JSON with relevancy scores for each policy.
Score each policy from 0-100 based on how relevant its title is to the pattern context.
Order the list from most to least relevant. Do not invent policy IDs."""
...
Score each policy's relevance to the pattern from 0-100:
- 80-100: Highly relevant (title directly references CPT/DRG codes or clinical concept)
- 60-79: Moderately relevant (title relates to the clinical area or context)
- 40-59: Somewhat relevant (title has tangential connection)
- 0-39: Low relevance (title appears off-topic or unrelated)

Prefer policies whose titles reference the CPT/DRG codes, the clinical concept in the
pattern, or the LOB/state context.

Return ONLY JSON in this format:
{{
  "policy_scores": [
    {{"policy_id": "<id>", "relevancy_score": <0-100>, "reasoning": "<brief explanation>"}},
    ...
  ],
  "selected_policy_ids": ["<id1>", "<id2>", ...]
}}

Order both lists from most to least relevant."""
...
POLICY_RELEVANCY_THRESHOLD = 60  # Minimum relevancy score (0-100) to include a policy


# Word-level fallback for the title-anchor filter. Triggered when the strict
# multi-word regex is too aggressive (drop >80% AND <30 rows remain). Words
# below length 3 or in the stopword set are discarded before OR-matching.
...
def _extract_anchor_words(anchors: List[str]) -> List[str]:
    """Split multi-word title anchors into individual meaningful words for
    fallback word-level matching. Filters stopwords and words shorter than
    3 characters; dedups case-insensitively while preserving first-seen
    casing."""
    words: List[str] = []
    seen: set = set()
...
class ReimbursementPolicyAgent(AgentBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Pass llm_timeout to base class - it will be stored as self.llm_timeout
        super().__init__(
        ...
        self.llm_nano = self._init_llm("nano")
        ...
        self.llm_mini = self._init_llm("mini")
    ...
    def _init_llm(self, model_type: str = "base") -> Any:
        """
        Initialize LLM with specified model configuration.

        Args:
            model_type: Type of model to initialize ('base', 'mini', or 'nano')
                - 'base': GPT-5.4-mini with medium reasoning (for retry operations)
                - 'mini': GPT-5.4-nano with low reasoning (for fast policy extraction)
                - 'nano': GPT-5.4-nano with low reasoning (alias for 'mini')

        Returns:
            Configured ChatOpenAI instance
        """
        model_configs = {
            "base": {"model": "gpt-5.4", "reasoning": "high"},
            "mini": {"model": "gpt-5.4-mini", "reasoning": "medium"},
            "nano": {"model": "gpt-5.4-nano", "reasoning": "low"},
        }

        if model_type not in model_configs:
            self.logger.warning(f"Unknown model type '{model_type}', defaulting to 'base'")
            model_type = "base"
        ...
        try:
            ...
            llm = ChatOpenAI(
                model=config["model"],
                api_key=token,
                extra_body={"reasoning_effort": config["reasoning"]}
            )

            self.logger.info(
                f"Initialized {model_type} LLM: {config['model']} "
                f"with reasoning_effort='{config['reasoning']}'"
            )
            return llm

        except Exception as e:
            self.logger.warning(f"Failed to initialize {model_type} LLM: {e}")
            # Fallback to main LLM if initialization fails
            return getattr(self, 'llm', None) or self._initialize_llm()
    ...
    def _invoke_llm_with_retry(self, llm: Any, messages: list, **invoke_kwargs) -> Any:
        ...
        of the agent's three LLMs (`self.llm` / `self.llm_nano` /
        `self.llm_mini`) was passed.
        ...
        """
        # Extract schema parameter for logging (not passed to LLM)
        schema = invoke_kwargs.pop('schema', 'unknown')
        ...
        if llm is self.llm:
            result = self._invoke_with_token_retry(messages, **invoke_kwargs)
            return result

        # Mini / retry — base helper is hardcoded to self.llm, so use
        # `ehap_retry.llm_invoke` directly with the matching tier factory.
        if llm is getattr(self, "llm_nano", None):
            def reinitializer():
                return self._init_llm("mini")
            llm_role = "mini"
        elif llm is getattr(self, "llm_mini", None):
            def reinitializer():
                return self._init_llm("base")
            llm_role = "retry"
        else:
    ...
    def search_policies_node(self, state: ReimbursementState) -> Dict[str, Any]:
        ...
        # Short-circuit when the pattern gave us neither CPT/HCPCS codes nor
        # generable keywords. Calling the policy API with keyword=Unknown
        # 404s upstream and burns downstream tokens on empty content, so
        # signal insufficient context and let downstream nodes pass through.
        if not search_keywords and not cpt_codes:
            self.logger.warning(
                f"Pattern {pattern_rank}: insufficient search context "
                f"(no CPT/HCPCS codes and no search keywords). Skipping policy API call."
            )
            return {
                "raw_policies": [],
                "filtered_policies": [],
                "search_context_status": "insufficient",
            }

        if search_keywords:
            # search_keywords is a list of keywords
            # Use first two keywords on first attempt, all three on retry
            keyword_for_api = None  # Will be set based on attempt
            self.logger.info(f"Pattern {pattern_rank}: Available keywords: {search_keywords}")
        else:
        ...
        if pattern:
            ...
            if lob_list:
                # Collapse business-unit-qualified LOB variants to the canonical
                # LOB names the policy_comparison API expects. These variants
                # come from the semantic view's line_of_business_desc enum
                # (see pi_wgs_mad_semantic_view_with_samples_local.yaml line 84:
                # sample values include "GBD Medicaid", "GBD Medicare",
                # "Commercial Individual", "Commercial Local Group",
                # "Medicare GRS", "Medicare Indiv") — the API rejects those
                # verbatim and only accepts the top-level LOB name.
                for index, lob in enumerate(lob_list):
                    if lob in ["Commercial Individual", "Commercial Local Group"]:
                        lob_list[index] = "Commercial"
                    elif lob in ["Medicare GRS", "Medicare Indiv", "GBD Medicare"]:
                        lob_list[index] = "Medicare"
                    elif lob == "GBD Medicaid":
                        lob_list[index] = "Medicaid"
        ...
        for attempt in range(max_retries + 1):
            # Set keyword based on attempt number
            if search_keywords:
                if attempt == 0:
                    # First attempt: use first two keywords
                    keywords_to_use = search_keywords[:2]
                    keyword_for_api = ', '.join(keywords_to_use)
                    self.logger.info(f"Pattern {pattern_rank}: First attempt - using first 2 keywords: '{keyword_for_api}'")
                else:
                    # Retry attempts: use all three keywords
                    keyword_for_api = ', '.join(search_keywords)
                    self.logger.info(f"Pattern {pattern_rank}: Retry attempt {attempt} - using all keywords: '{keyword_for_api}'")
        ...
        self.logger.info(f"Pattern {pattern_rank}: API returned {len(raw_data.get('sentencelist', 0))} policies (with keyword + filters)")
        
        # Convert to DataFrame for additional filtering
        df = pd.DataFrame(raw_data)
        df = df['sentencelist'].apply(pd.Series)
        
        # Log original payor distribution before any filtering
        original_payors = df['payor'].value_counts().to_dict()
        self.logger.info(f"Pattern {pattern_rank}: Original payors from API: {original_payors}")
        
        # Check for Elevance in original API response
        elevance_in_api = df['payor'].str.contains('Elevance', case=False, na=False).any()
        if elevance_in_api:
            elevance_count = df['payor'].str.contains('Elevance', case=False, na=False).sum()
            elevance_types = df[df['payor'].str.contains('Elevance', case=False, na=False)]['policy_type'].value_counts().to_dict()
            self.logger.info(f"Pattern {pattern_rank}: Found {elevance_count} Elevance policies in API response with types: {elevance_types}")
        ...
        self.logger.info(f"Pattern {pattern_rank}: After PDF/policy_type/dedup: {len(df)} policies")

        # Title-anchor topical filter — drops policies whose title doesn't
        # match any topical anchor for this code. Fixes the "policy mentions
        # 'ambulance' in prose but is really about Modifier Usage" problem
        # (see scripts/POLICY_API_STRATEGIES.md Part 2). Skipped gracefully
        # when anchors weren't generated (legacy mode / LLM failure).
        # If the filter would drop ALL policies (over-specific anchors), we
        # revert to the pre-filter df so downstream _apply_policy_caps can
        # still triage — otherwise we'd hand an empty policy_ids list to
        # _get_policy_hashes and crash on `WHERE PLCY_ID IN ()`.
        title_anchors = state.get("title_anchors") or []
        if title_anchors and "policy_title" in df.columns and len(df) > 0:
            ...
            if usable:
                ...
                if len(filtered) == 0:
                    self.logger.warning(
                        f"Pattern {pattern_rank}: title-anchor filter dropped all "
                        f"{before} policies (anchors={usable}) — reverting to "
                        f"pre-filter set so LLM triage can still run"
                    )
                else:
                    drop_pct = (before - len(filtered)) / before
                    # Word-level fallback when the strict multi-word regex is
                    # over-restrictive (e.g. anchor "Ambulance Transportation"
                    # misses "Ambulance Services"). Trigger only when both the
                    # drop is severe AND the result is tiny — protects small
                    # input sets from spurious fallback (4 → 1 is 75% but only
                    # starts from 4 rows).
                    if drop_pct > 0.80 and len(filtered) < 30:
                        ...
                        if words:
                            ...
                            if len(fallback) > len(filtered):
                                self.logger.info(
                                    f"Pattern {pattern_rank}: strict title-anchor filter "
                                    f"too aggressive (anchors={usable}: {before} → {len(filtered)}, "
                                    f"drop {drop_pct*100:.0f}%); falling back to word-level "
                                    f"match (words={words}): {before} → {len(fallback)} policies"
                                )
                                df = fallback
                        ...
                        if not fallback_applied:
                            df = filtered
                            self.logger.info(
                                f"Pattern {pattern_rank}: title-anchor filter "
                                f"(anchors={usable}): {before} → {len(df)} policies "
                                f"(fallback attempted but did not improve)"
                            )
                    else:
                        df = filtered
                        self.logger.info(
        ...
        # Collapse state fanout: dedup by (payor, policy_title). Elevance
        # publishes identical titles per state, so 200 policy_ids can
        # collapse to ~20 unique (payor, title) pairs. Keep the row with
        # the highest policy_score per group; falls back to first
        # occurrence when scores are missing or non-numeric (API returned
        # score-descending order, so keep="first" is safe).
        if "policy_title" in df.columns and len(df) > 0:
            ...
            self.logger.info(
                f"Pattern {pattern_rank}: (payor, title) dedup: {before} → {len(df)} policies"
            )

        # Verify Elevance survived filtering
        if elevance_in_api:
            elevance_after_filter = df['payor'].str.contains('Elevance', case=False, na=False).sum()
            if elevance_after_filter > 0:
                self.logger.info(f"Pattern {pattern_rank}: ✓ Retained {elevance_after_filter} Elevance policies after filtering")
            else:
                self.logger.warning(f"Pattern {pattern_rank}: ⚠ Lost all Elevance policies during filtering!")
        ...
        # Check for both internal and external Elevance policies (informational only)
        internal_elevance_payors = ['Elevance Health (Internal)', 'Elevance Health (internal)']
        has_internal_elevance = df['payor'].isin(internal_elevance_payors).any()
        has_external_elevance = (df['payor'] == 'Elevance Health (external)').any()
        
        if has_internal_elevance and has_external_elevance:
            internal_count = len(df[df['payor'].isin(internal_elevance_payors)])
            external_count = len(df[df['payor'] == 'Elevance Health (external)'])
            self.logger.info(
                f"Pattern {pattern_rank}: Found both internal ({internal_count}) and external ({external_count}) "
                f"Elevance policies - keeping both as they may contain different information"
            )
        elif has_internal_elevance:
            internal_count = len(df[df['payor'].isin(internal_elevance_payors)])
            self.logger.info(f"Pattern {pattern_rank}: Found {internal_count} internal Elevance policy(ies)")
        elif has_external_elevance:
            external_count = len(df[df['payor'] == 'Elevance Health (external)'])
            self.logger.info(f"Pattern {pattern_rank}: Found {external_count} external Elevance policy(ies)")
    ...
    def fetch_content_node(self, state: ReimbursementState) -> Dict[str, Any]:
        ...
        """
        if not state.get("filtered_policies"):
            self.logger.info("fetch_content: no filtered policies, skipping Snowflake query")
    ...
    def extract_rules_node(self, state: ReimbursementState) -> Dict[str, Any]:
        ...
        """
        policy_content = state.get("policy_content") or []
        filtered_policies = state.get("filtered_policies") or []
        if not filtered_policies or not policy_content:
            self.logger.info("extract_rules: no policies to extract, returning empty result")
            return {"result": []}

        cpt_codes = state["cpt_codes"]
        drg_codes = state.get("drg_codes", [])
        pattern = state.get("pattern", {})
        llm_nano = state["llm_nano"]  # GPT-5.4-nano — first attempt
        ...
        llm_mini = state.get("llm_mini") or llm_nano
        ...
        def _process_one(idx: int, policy: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            ...
            None on terminal failure. Per-policy retry uses llm_mini."""
            ...
            for attempt in range(1, total_attempts + 1):
                try:
                    # Dynamically switch to retry LLM after the first failure
                    current_llm = llm_nano if attempt == 1 else llm_mini
                    ...
                    if attempt == total_attempts:
                        self.logger.error(
                            f"Extract failed after final attempt ({attempt}/{total_attempts}) "
                            f"for policy_id={policy_id} title={policy_title!r}: {e}; dropping policy"
                        )
                        return None
                    
                    # Handle intermediate failure / retry logging
                    self.logger.warning(
                        f"Extract attempt {attempt} failed for policy_id={policy_id} "
                        f"title={policy_title!r}: {e}; retrying after {self.retry_delay}s"
                    )
    ...
    def validate_policies_node(self, state: ReimbursementState) -> Dict[str, Any]:
        ...
        filtered_policies = state.get("filtered_policies") or []
        if not results:
            self.logger.info("validate_policies: no results to validate, passthrough")
            return {
                "result": [],
        ...
        # `llm_nano` powers the judge; tests that build the agent via
        ...
        # when an llm_nano is provided AND we have non-empty evidence
        ...
        if llm_nano is not None and policy_contexts:
    ...
    def _judge_policy_relevance(
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="relevance_judge")
    ...
    def analyze_table_structure_node(self, state: ReimbursementState) -> Dict[str, Any]:
        ...
        if not results:
            self.logger.info("analyze_table_structure: no results, returning empty schema")
            return {
    ...
    def format_output_node(self, state: ReimbursementState) -> Dict[str, Any]:
        ...
        if not results or not filtered_policies:
            self.logger.info(
                "format_output: no results/policies, emitting empty formatted output"
            )
            empty_summary = {
                "title": "Payer Policy Summary",
                "subtitle": self._resolve_pattern_title(pattern),
                "columns": [],
                "rows": [],
            }
    ...
    def generate_recommendation_node(self, state: ReimbursementState) -> Dict[str, Any]:
        ...
        if not formatted_output.get("individual_policies"):
            self.logger.info(
                "Skipping recommendation generation (no individual policies to summarize)"
            )
            return {"recommended_action": [], "elevance_executive_summary": None}
    ...
    def _generate_column_labels(
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="column_labels")
    ...
    def _aggregate_payer_rules(
        ...
        prompt = textwrap.dedent(f"""
            Summarize these policy rules for {payer_name} into ONE concise statement (max 15 words):
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="table_schema")
    ...
    def _extract_drg_codes_from_cards(
        ...
        semantic_roles: Optional[Dict[str, str]] = None,
    ) -> List[str]:
        """
        Extract DRG codes / names from source cards.

        When ``semantic_roles`` is provided, any filter whose field maps to
        ``drg_code`` or ``drg_name`` in the classifier taxonomy is treated
        as a DRG source, regardless of the physical column name. Falls back
        to the historical ``field == 'drg_name'`` check when no roles are
        supplied or the field isn't in the mapping.

        Args:
            source_card_ids: List of card IDs to extract from
            cards: Full list of cards
            semantic_roles: Optional dim_name -> classifier role mapping
        ...
        drgs = set()
        role_map = semantic_roles or {}
        drg_roles = {"drg_code", "drg_name"}
        ...
        for card_id in source_card_ids:
            ...
            # Method 1: Check filters for DRG fields
            filters = card.get('filters', [])
            for filter_item in filters:
                if not isinstance(filter_item, dict):
                    continue
                raw_field = filter_item.get('field', '') or ''
                role = role_map.get(raw_field) or role_map.get(raw_field.lower())
                if role is not None:
                    if role in drg_roles:
                        drg_value = filter_item.get('value')
                        if drg_value:
                            drgs.add(drg_value)
                    continue
                # TODO(remove-after-companion-jsons-committed): drop this
                # keyword fallback once every YAML has a semantic-roles JSON.
                if raw_field == 'drg_name':
                    drg_value = filter_item.get('value')
                    if drg_value:
                        drgs.add(drg_value)
    ...
    def _extract_cpt_codes_from_cards(
        ...
        """
        Extract CPT codes from source cards.

        When ``semantic_roles`` is provided (LLM-classified dim -> role
        mapping from the active YAML), a filter's field is treated as a CPT
        source when its role is one of ``procedure_code``, ``procedure_name``,
        or ``revenue_code``. This avoids the "src_provider_zip_code contains
        'code'" style false positives the keyword substring check produces.
        Falls back to the keyword substring check when no roles are supplied
        or the field isn't in the mapping.
        ...
        cpt_codes = set()
        role_map = semantic_roles or {}
        cpt_roles = {"procedure_code", "procedure_name", "revenue_code"}
        ...
        for card_id in source_card_ids:
            ...
            for filter_item in filters:
                ...
                role = role_map.get(raw_field) or role_map.get(field)
                if role is not None:
                    if role in cpt_roles:
                        value = str(filter_item.get('value', ''))
                        codes = re.findall(r'\b\d{5}\b', value)
                        cpt_codes.update(codes)
                    # A field with a known role that is NOT a CPT source
                    # (e.g. `zip`, `state`) is skipped — the point of using
                    # roles is exactly to avoid the keyword false positive.
                    continue
                # TODO(remove-after-companion-jsons-committed): drop this
                # keyword fallback once every YAML has a semantic-roles JSON.
                if 'cpt' in field or 'procedure' in field or 'hcpcs' in field:
    ...
    def _resolve_pattern_title(self, pattern: Dict[str, Any]) -> str:
        """Resolve a human-readable pattern title from pattern-agent output.

        Resolution order matches the pattern agent's field aliases:
        pattern_title → top_pattern → title → "Unknown Pattern".
        """
        if not pattern:
    ...
    def _extract_cpt_codes_from_pattern(
        ...
        2. pattern.key_driver_codes filtered by CPT/HCPCS regex
        3. Extract from source cards using _extract_cpt_codes_from_cards()
        ...
        # 2. Filter pattern.key_driver_codes for valid CPT/HCPCS codes.
        # Pattern agent emits mixed strings here (e.g. "A0429", "Out-of-network",
        # "Ground Ambulance"); re.fullmatch rejects the free-text entries so we
        # only forward real codes to the downstream API.
        key_driver_codes = pattern.get("key_driver_codes", [])
        if isinstance(key_driver_codes, list) and key_driver_codes:
            valid_codes = sorted({
                c for c in key_driver_codes
                if isinstance(c, str) and re.fullmatch(r"[A-Z]?\d{4,5}", c)
            })
            if valid_codes:
                result = ",".join(valid_codes)
                self.logger.info(
                    f"Extracted {len(valid_codes)} CPT/HCPCS codes from "
                    f"pattern.key_driver_codes: {result}"
                )
                return result

        # 3. Extract from source cards
        source_card_ids = pattern.get("source_card_ids", [])
        cards = context.get("cards", [])

        if source_card_ids and cards:
            cpt_codes = self._extract_cpt_codes_from_cards(
                source_card_ids, cards, semantic_roles=semantic_roles
            )
            if cpt_codes:
        ...
        self.logger.warning(
            ...
            f"Checked pattern.cpt_codes, pattern.key_driver_codes, and source cards."
        )
        return ""
    ...
    def _extract_lob_and_state_filters(
        ...
        """
        Extract LOB and State filters from source cards' canonical_dimensions.

        The pattern_agent role mapping is driven by the active semantic view
        (e.g. ``configs/correlation_pattern/pi_wgs_mad_semantic_view_with_samples_local.yaml``),
        so we do NOT hard-code semantic dimension names or state/LOB values
        here. Instead we read cards by ROLE:

        - ``canonical_dimensions['state']`` — populated from provider_state_code,
          prov_service_state, rendering_state, member_brand_state, etc. This
          role is intentionally distinct from ``geography`` so ZIP codes
          (src_provider_zip_code) don't leak into the state filter.
        - ``canonical_dimensions['line_of_business']`` — populated from
          line_of_business_desc, wellpoint_line_of_business_full_cde, etc.

        ``priority_entities.states`` is only trusted when the entry is also
        attested by at least one source card's state-role bucket. This
        guards against upstream LLM leaks (program names, ZIP codes) that
        occasionally slip past the pattern_agent's own filter.
        ...
        source_card_ids = pattern.get('source_card_ids', []) or []
        cards = context.get('cards', []) or []

        card_state_values: set = set()
        if source_card_ids and cards:
            card_lookup = {card.get('card_id'): card for card in cards if card.get('card_id')}
            ...
            for card_id in source_card_ids:
                ...
                canonical_dims = card.get('canonical_dimensions') or {}

                for value in canonical_dims.get('line_of_business', []) or []:
                    if value:
                        lobs.add(str(value))

                for value in canonical_dims.get('state', []) or []:
                    if value:
                        card_state_values.add(str(value))

        # Trust priority_entities.states only for values that also appear
        # in a card's state-role bucket. When cards don't attest any state
        # (e.g. non-state-scoped patterns), fall back to card-derived
        # values only — which will be empty, matching the "no state filter"
        # intent. Never send unvalidated LLM-emitted values downstream.
        priority_entities = pattern.get('priority_entities') or {}
        pattern_states = priority_entities.get('states') or []
        if card_state_values:
            for value in pattern_states:
                if value and str(value) in card_state_values:
                    states.add(str(value))
            # If the LLM didn't emit any card-attested state (or emitted
            # only leaked non-state values), still surface the card
            # values themselves so a state-scoped card yields a state filter.
            if not states:
                states.update(card_state_values)
        ...
        self.logger.info(
            ...
            f"State={state_list} from card canonical_dimensions.state "
            f"(intersected with pattern.priority_entities.states)"
        )
    ...
    def _generate_search_keywords_from_drg(
        ...
        Uses LLM to identify the top 3 most relevant keywords that best describe
        ...
        Returns:
            List of keywords (top 3) or fallback list from first DRG code
        """
        if not drg_codes:
            return []
        ...
        keyword_prompt = textwrap.dedent(f"""
            You are translating DRG billing codes into clinical search terms for a text-based policy search engine.

            DRG code descriptions: {drg_codes_str}

            {user_question_block}

            CORE PROBLEM: Medical reimbursement policies use natural clinical language (e.g., "heart failure", "pneumonia"), but DRG codes contain administrative billing jargon (e.g., "W MCC", "CC", "W/O", "AGE > 17") that NEVER appears in policy text.

            YOUR TASK:
            1. Extract the core CLINICAL CONDITION or PROCEDURE from each DRG description.
            2. Strip ALL billing modifiers: MCC, CC, W/O, W/, WITHOUT, WITH, age thresholds, severity indicators.
            3. Convert administrative abbreviations to full clinical terms that appear in medical policies.
            4. Generate a single list of the top three distinct relevant clinical keywords based on the provided DRGs.

            STRICT RELEVANCE & ORDERING RULES:
            - Return exactly 3 most relevant keywords as a comma-separated list.
            - Keywords must be strictly ordered from most relevant to least relevant (the first item must be more relevant than the second item, and so on).
            - Return ONLY clinical terms that would appear in narrative medical policy documents.
            - Use natural medical language, not billing codes or regulatory titles.
            - Each keyword must be 1-2 words maximum.
            - Avoid generic terms: "surgery", "procedure", "inpatient", "MS DRG", "surgical".
            - Focus on the specific medical condition, body system, or clinical procedure.

            EXAMPLES:
            - DRG "291 - Heart Failure & Shock W MCC" → "heart failure, cardiogenic shock, heart disease"
            - DRG "193 - Simple Pneumonia W CC" → "pneumonia, lung infection, pneumonitis"
            - DRG "470 - Major Hip Joint Replacement" → "hip replacement, joint arthroplasty, hip arthroplasty"
            - DRG "189 - Pulmonary edema and respiratory failure" → "respiratory failure, pulmonary edema, lung failure"

            Return ONLY the final 3 comma-separated keywords, nothing else.""")
        ...
        try:
            keyword_response = self._invoke_llm_with_retry(llm, keyword_messages, schema="search_keywords")
            ...
            search_keywords_list = [k.strip() for k in search_keywords_raw.split(',')]
            # search_keywords = ', '.join(keywords_list[:2])

            self.logger.info(f"Generated keywords from {len(drg_codes)} DRG codes: '{search_keywords_list}' (LLM)")
            return search_keywords_list
    ...
    def _generate_search_keywords_from_cpt(
        self,
        cpt_codes: str,
        llm: Any,
        user_query: Optional[str] = None
        ...
        """
        Generate smart search keywords from CPT/HCPCS codes using LLM.

        Motivation: the Carelon policy comparison API does exact-string
        matching on document text. Reimbursement policies often describe
        rules narratively ("Ambulance and Non-Emergent Transport") without
        ever spelling out the specific procedure code, while separate
        Clinical Guidelines documents contain the HCPCS code table. That
        cross-document join is invisible to a keyword=<code>-only search:
        e.g. searching `a0427` finds fee schedules and provider manuals
        that list the code but drops the 200 Elevance Reimbursement
        ambulance policies entirely. Combining `<code>,<clinical_topic>`
        (e.g. `a0427,ambulance`) is strictly superior — it adds the
        narrative policies while retaining all code-listing matches.

        This mirrors `_generate_search_keywords_from_drg` but is scoped to
        CPT/HCPCS codes: ask the LLM for a 1-2-word clinical topic
        describing what the codes cover, then return
        `<first_code>,<clinical_topic>` for the API keyword parameter.

        Args:
            cpt_codes: Comma-separated CPT/HCPCS codes (e.g. "A0427" or "99291,99292")
            llm: LLM client for keyword generation
            user_query: Optional user question used only as a tie-breaker signal;
                codes remain the primary input.

        Returns:
            String of the form `<code>,<clinical_topic>` (e.g. `A0427,ambulance`),
            or the raw first code on error (preserves prior fallback behavior).
        """
        if not cpt_codes:
            return ""

        first_code = cpt_codes.split(",")[0].strip()
        code_list = [c.strip() for c in cpt_codes.split(",") if c.strip()]
        codes_str = json.dumps(code_list)

        cleaned_query = self._clean_user_query(user_query)
        user_question_block = (
            f"\nUser question (use ONLY as a tie-breaker to bias between equally-relevant themes; "
            f"do not derive the keyword from this alone):\n{cleaned_query}\n"
            if cleaned_query else ""
        )

        # Prompt guidance is tuned from the 48-scenario API sweep in
        # scripts/test_policy_api_strategies.py — a broader category term
        # (what payors actually put in policy TITLES) consistently returns
        # 2-5× more Elevance Reimbursement policies than the narrow
        # clinical service term. Examples from the sweep (Elevance Reimb
        # count with `code,keyword`): 99291 → "critical care"=277 vs
        # "evaluation and management"=1359; J3490 → "unclassified drug"=107
        # vs "injection"=181; 90837 → "psychotherapy"=25 vs
        # "behavioral health"=144; A0427 → "ambulance"=200 vs
        # "transportation"=230.
        keyword_prompt = f"""Given the following CPT/HCPCS procedure codes: {codes_str}
{user_question_block}
Identify the SINGLE most relevant clinical CATEGORY keyword (1-2 words) that describes what these codes cover for searching reimbursement policies.

Prefer the broader clinical CATEGORY name that would appear in a payor's reimbursement policy TITLE — not the narrowest possible clinical noun. Reimbursement policies are typically published per-category (e.g. "Ambulance and Non-Emergent Transport", "Evaluation and Management Services", "Behavioral Health Services", "Diagnostic Imaging", "Injectable Drugs"), and matching at the category level recovers narrative policies that never spell out the specific code.

Guidance on category level:
- For ambulance/EMS codes (A04xx-A09xx) prefer "ambulance" or "transportation".
- For E/M codes (99xxx) prefer "evaluation and management" over specific service names.
- For behavioral health codes (90xxx-908xx) prefer "behavioral health" over "psychotherapy".
- For J-codes (drugs/biologicals) prefer "injection" or "drug administration" over specific drug names.
- For imaging codes (7xxxx) prefer "diagnostic imaging" or the modality ("CT", "MRI") over the anatomical target.
- For surgery codes prefer the surgical CATEGORY (e.g. "spine surgery", "orthopedic surgery") over the specific procedure.
- For GI/endoscopy codes (43xxx) prefer "endoscopy" over "upper endoscopy".
- For hospital outpatient codes (G04xx) prefer "hospital outpatient" or "outpatient services".

Do not use special characters — only words and phrases.
Avoid overly generic terms such as "procedure", "service", "professional service", "medical service" which return too many irrelevant policies.
If the codes span multiple themes, pick the theme covering the most codes.

Return ONLY the single keyword phrase, nothing else. Examples: "ambulance", "evaluation and management", "behavioral health", "diagnostic imaging" """
        ...
        try:
            ...
            self._record_tokens("search_keywords", keyword_response)
            keyword_raw = keyword_response.content.strip().strip('"').strip("'")
            # Guard against multi-line or comma-separated responses — the DRG
            # helper returns 2 keywords; here we want exactly 1 clinical topic.
            keyword = keyword_raw.split(",")[0].split("\n")[0].strip()

            if not keyword:
                self.logger.warning(
                    f"LLM returned empty CPT keyword for codes {code_list}; "
                    f"falling back to first code alone: '{first_code}'"
                )
                return first_code

            combined = f"{first_code},{keyword}"
            self.logger.info(
                f"Generated keyword from {len(code_list)} CPT/HCPCS code(s): "
                f"'{combined}' (LLM topic='{keyword}')"
            )
            return combined

        except Exception as e:
            self.logger.warning(
                f"LLM keyword generation for CPT/HCPCS failed: {e}; "
                f"falling back to first code alone: '{first_code}'"
            )
            return first_code

    def _generate_title_anchors_from_cpt(
        self,
        cpt_codes: str,
        llm: Any,
        user_query: Optional[str] = None,
        ...
        """
        Generate 2-5 title-substring anchors for filtering policies by topical
        relevance to the CPT/HCPCS codes.

        Motivation: even with the `code,broad_category` keyword strategy that
        recovers narrative Reimbursement policies, the API returns huge sets
        of policies that mention the keyword only in prose. For J3490,injection
        it returns 722 Reimbursement policies of which 181 are Elevance, but
        most are off-topic ("Injection Therapy for Headache", "Preadmission
        Services", "Modifier Usage" — they just contain the word "injection"
        somewhere). Applying a title-anchor regex drops 90%+ of those false
        positives while preserving all topically-relevant Elevance policies —
        measured across 8 codes in scripts/test_policy_api_reduction.py
        (12-100× reduction in Reimbursement, 5-72× in Elevance).

        Args:
            cpt_codes: Comma-separated CPT/HCPCS codes (e.g. "A0427" or
                "99291,99292").
            llm: LLM client for anchor generation (typically self.llm_mini).
            user_query: Optional user question forwarded as a tie-breaker.

        Returns:
            List of 2-5 anchor terms. Caller compiles them into a
            case-insensitive alternation regex against `policy_title`.
            Returns [] on empty input or any LLM failure — the caller
            must skip the title-regex step in that case (dedup still runs).
        """
        if not cpt_codes or llm is None:
            return []
        ...
        cleaned_query = self._clean_user_query(user_query)
        user_question_block = (
            f"\nUser question (use ONLY as a tie-breaker; do not derive anchors from this alone):\n{cleaned_query}\n"
            if cleaned_query else ""
        )

        # Anchor examples in the prompt are drawn from Elevance's own title
        # catalog (see scripts/elevance_title_taxonomy.json and the
        # 1462-title corpus) so the LLM biases toward multi-word domain
        # nouns that actually appear in Reimbursement policy titles.
        anchor_prompt = f"""Given the following CPT/HCPCS procedure codes: {codes_str}
{user_question_block}
List 2-5 short title-substring anchors (each 1-3 words) that would appear in the TITLE of a payor's Reimbursement policy for these codes. These will be OR-matched (case-insensitive) against policy titles to drop off-topic policies that only mention the code/category in prose.

Guidance:
- Prefer SHORT anchors (1-3 words). These are substring matches against actual policy titles — a longer anchor like "Ambulance Transportation Services" will NOT match a policy titled "Ambulance Transportation" because the anchor exceeds the title. When in doubt, drop qualifier words.
- Bare single words like "injection" or "endoscopy" can over-match. Prefer a compact 2-word domain noun ("Ambulance Transportation", "Injection Administration", "Endoscopy Multiple") over either extreme.
- Include 1-2 synonyms or common title variants (e.g. "Ambulance" and "Transportation" both anchor A0427).
- Include cross-cutting terms only if the code is a surgical/procedural code where modifier-family policies (Multiple Surgery, Assistant Surgery, Co-Surgeon, Global Surgery) are the most relevant policies.
- Do not include the raw code number.
- Do not use special characters — only words, spaces, and hyphens.

Examples (for orientation, not to be reused verbatim):
- A0427 → Ambulance, Transportation, Non-Emergent
- 99291 → Critical Care, Evaluation Management, Emergency Department
- 90837 → Psychotherapy, Behavioral Health, Mental Health
- J3490 → Drugs Biologicals, Unlisted Code, Injection Administration
- 22551 → Spine, Multiple Surgery, Assistant Surgery

Return ONLY the anchors, one per line, no numbering, no bullets, no explanations."""

        anchor_messages = [
            {"role": "system", "content": "You are a medical coding expert. Generate concise title anchors."},
            {"role": "user", "content": anchor_prompt},
        ]

        try:
            response = self._invoke_llm_with_retry(llm, anchor_messages)
            self._record_tokens("title_anchors", response)
            ...
            for line in raw.splitlines():
                # Strip bullet/number prefixes the LLM may add despite instructions.
                s = line.strip().lstrip("-*•").strip()
                s = re.sub(r"^\d+[.)]\s*", "", s)
                s = s.strip('"').strip("'").strip()
                # Guard: no code numbers, no empty strings, dedup case-insensitively.
                if not s:
                    continue
            ...
            if not deduped:
                self.logger.warning(
                    f"LLM returned no usable title anchors for codes {code_list}; "
                    f"skipping title-regex step (dedup will still run)"
                )
                return []

            self.logger.info(
                f"Generated {len(deduped)} title anchor(s) from {len(code_list)} "
                f"CPT/HCPCS code(s): {deduped}"
            )
            return deduped

        except Exception as e:
            self.logger.warning(
                f"LLM title-anchor generation failed: {type(e).__name__}: {e}; "
                f"skipping title-regex step (dedup will still run)"
            )
            return []
    ...
    def _generate_search_keywords(
        ...
        Uses LLM to generate smart keywords from either code system so the API
        keyword parameter carries a clinical topic alongside the code — the
        policy_comparison API matches exact strings, so a raw code alone misses
        narrative policies that describe rules by clinical topic rather than
        by code (see _generate_search_keywords_from_cpt for the full rationale).

        LLM tier selection:
          - DRG path uses `llm` (main tier, GPT-5.4). DRG descriptions can be
            long and multi-code; picking "the top 2 themes across a set" needs
            more reasoning than the CPT case.
          - CPT path uses `llm_mini` (GPT-5.4-nano) when provided, falling back
            to `llm` otherwise. Mapping a code to a 1-2-word clinical category
            is a small classification task that nano handles reliably at ~10×
            the throughput and a fraction of the cost of the main tier.

        Args:
            drg_codes: List of DRG codes from pattern
            cpt_codes: Comma-separated CPT codes
            llm: Main LLM (GPT-5.4). Used for DRG keyword generation; also
                used as the CPT fallback when `llm_mini` is None.
            user_query: Optional user question forwarded to the keyword
                generator as a tie-breaker signal.
            llm_mini: Optional smaller LLM (GPT-5.4-nano). When provided, the
                CPT path routes through it instead of `llm`. Preferred for
                cost/latency reasons.

        Returns:
            List of search keywords for policy API
        """
        # Prioritize DRG codes for search with LLM-based keyword generation
        if drg_codes and llm:
            search_keywords = self._generate_search_keywords_from_drg(drg_codes, llm, user_query=user_query)
        ...
        if drg_codes:
            ...
            meaningful = [w for w in words if len(w) > 2 and w.lower() not in {'the', 'and', 'for'}]
            search_keywords = [' '.join(meaningful[:2])] if meaningful else [primary_drg]
            self.logger.info(f"Using extracted keywords from first DRG: {search_keywords}")
            return search_keywords

        # CPT/HCPCS codes: use nano-tier LLM to enrich the raw code with a
        # clinical topic so narrative Reimbursement policies (which don't
        # spell out the code) are also matched. This is a small classification
        # task well within nano's capabilities — falls back to the main LLM
        # only when nano wasn't provided, and to the raw first code when no
        # LLM is available at all (preserves pre-existing legacy behavior).
        cpt_llm = llm_mini or llm
        if cpt_codes and cpt_llm:
            cpt_result = self._generate_search_keywords_from_cpt(
                cpt_codes, cpt_llm, user_query=user_query
            )
            return [cpt_result]
        if cpt_codes:
            first_cpt = cpt_codes.split(",")[0].strip()
            self.logger.info(f"No LLM; using raw first CPT code as search keyword: {first_cpt}")
            return [first_cpt]
    ...
    def _generate_policy_recommendations(
        ...
        prompt = textwrap.dedent(f"""
            You are generating a code-specific reimbursement policy recommendation.
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="policy_recommendations")
    ...
    def _run_recommendation_validator(
        ...
        prompt = textwrap.dedent(f"""
            You are reviewing draft reimbursement-policy recommendation items for fit with the user's pattern and the cited sources.
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="validate_recommendation")
    ...
    def _get_policy_hashes(self, policy_ids: List[str]) -> pd.DataFrame:
        ...
        """
        # Short-circuit empty list — `WHERE PLCY_ID IN ()` is invalid SQL.
        if not policy_ids:
            return pd.DataFrame(columns=["PLCY_ID", "PDF_HASH_VAL_ID"])
    ...
    def _apply_policy_caps(
        ...
        if len(other_df) > 0 and llm_nano is not None:
            triage_result = self._triage_policy_titles(
                other_df=other_df,
            ...
            if triage_result:
                selected_ids, triage_scores = triage_result
                ...
                if ordered:
                    ...
                    triage_ran = True
                    # Apply budget cap only if we exceed it after threshold filtering
                    if len(other_df) > other_budget:
                        self.logger.info(
                            f"Pattern {pattern_rank}: Threshold filtering returned {len(other_df)} policies, "
                            f"capping to budget of {other_budget}"
                        )
                        other_df = other_df.head(other_budget)

        # Only apply head() if triage didn't run
        if not triage_ran:
            other_df = other_df.head(other_budget)
        ...
        if len(result) < before:
            score_info = ""
            if triage_scores:
                avg_score = sum(triage_scores.values()) / len(triage_scores)
                score_info = f", avg_relevancy={avg_score:.1f}"

            self.logger.warning(
                f"Pattern {pattern_rank}: Capped {before} → {len(result)} policies "
                f"(≤{POLICY_CAP_MAX_PER_PAYOR}/payor, ≤{POLICY_CAP_MAX_TOTAL} total, "
                f"Elevance pinned, triage_used={triage_ran}{score_info}). "
    ...
    def _triage_policy_titles(
        ...
        llm: Any,
        pattern_rank: Optional[int],
        relevancy_threshold: int = POLICY_RELEVANCY_THRESHOLD,
    ) -> Optional[tuple[List[str], Dict[str, int]]]:
        """Ask the mini LLM to rank non-Elevance policy_ids by title relevance.

        Returns a tuple of (selected_policy_ids, score_map) where:
        - selected_policy_ids: List of policy IDs meeting the relevancy threshold, ordered by score
        - score_map: Dict mapping policy_id to relevancy_score
        
        Returns None on any failure (LLM error, malformed JSON, empty selection,
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="triage_policies")
            ...
            # Build score map from policy_scores if available
            score_map = {}
            if validated.policy_scores:
                score_map = {
                    ps.policy_id: ps.relevancy_score
                    for ps in validated.policy_scores
                }
                ...
                filtered_ids = [
                    ps.policy_id
                    for ps in validated.policy_scores
                    if ps.relevancy_score >= relevancy_threshold
                ]

                if filtered_ids:
                    self.logger.info(
                        f"Pattern {pattern_rank}: Triage filtered {len(validated.policy_scores)} → "
                        f"{len(filtered_ids)} policies using threshold {relevancy_threshold}. "
                        f"Score range: {min(score_map.values())}-{max(score_map.values())}"
                    )
                    return (filtered_ids, score_map)

                self.logger.warning(
                    f"Pattern {pattern_rank}: No policies met relevancy threshold {relevancy_threshold}; "
                    f"falling back to API order"
                )
                return None
            
            # Fallback to selected_policy_ids if policy_scores not available
            ...
            # Use selected_policy_ids without scores (backward compatibility)
            self.logger.info(
                f"Pattern {pattern_rank}: Triage returned {len(validated.selected_policy_ids)} policies "
                f"(no scores available, using count-based selection)"
            )
    ...
    def _extract_policy_rules(
        ...
        response = self._invoke_llm_with_retry(llm, messages, schema="extract_rules")
    ...
    # CPT: 3-5 digits (includes DRGs), optional trailing letter (e.g. 99284, 99284T, 469).
    # HCPCS: 1 letter + 4 digits (e.g. G0378, A0427).
    ...
    # Ranges: Policies often specify code ranges (e.g. 99210-99220, A0426-A0428,
    # C50.0-C50.9). These are kept verbatim as single tokens.
    _CODE_RE = re.compile(
        r"^\d{3,5}[A-Z]?$"                                    # CPT/DRG single (3-5 digits, optional alpha suffix)
        r"|^[A-Z]\d{4}$"                                       # HCPCS Level II single (e.g. G0378)
        r"|^[A-Z]\d{2}(\.\d{1,4})?-?$"                        # ICD-10 single (e.g. O30.001, Z37.50-, C50)
        r"|^\d{3,5}[A-Z]?-\d{3,5}[A-Z]?$"                    # CPT/DRG range (e.g. 99210-99220, 469-470)
        r"|^[A-Z]\d{4}-[A-Z]\d{4}$"                           # HCPCS range (e.g. A0426-A0428)
        r"|^[A-Z]\d{2}(\.\d{1,4})?-[A-Z]\d{2}(\.\d{1,4})?$"  # ICD-10 range (e.g. C50.0-C50.9)
    )
    # Modifiers: 1-3 chars (digits or letters). Accept optional leading hyphen
    # for CMS ambulance origin/destination notation (e.g. -H, -I, -X).
    # The leading hyphen is stripped during normalization in _sanitize_field_values.
    _MODIFIER_RE = re.compile(r"^-?[A-Z0-9]{1,3}$", re.IGNORECASE)
    # Revenue codes: 3-5 digits with optional X wildcard or digit suffix (e.g. 0762, 045X, 068X).
    # Also accepts ranges (e.g. 0450-0459) for bundling rules.
    _REVENUE_CODE_RE = re.compile(
        r"^\d{3,5}[X0-9]?$"          # single code: 3-5 digits with optional X/digit suffix
        r"|^\d{2,4}X$"               # X-wildcard: e.g. 045X, 068X, 76X
        r"|^\d{3,5}-\d{3,5}$"        # range: e.g. 0450-0459, 762-769
    )
    ...
    def _sanitize_field_values(
        ...
        For modifier fields (required_modifiers, excluded_modifiers),
        normalizes values by stripping 'Modifier ' prefix, leading hyphens,
        and uppercasing (e.g. 'Modifier D' → 'D', '-H' → 'H', 'gt' → 'GT').
        """
        kept: List[str] = []
        is_modifier_field = field in ("required_modifiers", "excluded_modifiers")
        ...
        for raw in values or []:
            ...
            # For modifier fields, pre-normalize before validation:
            # Strip "Modifier " prefix (LLM sometimes extracts "Modifier D" instead of "D"),
            # strip leading hyphen (CMS ambulance notation), and uppercase.
            candidate = cleaned
            if is_modifier_field:
                # Strip case-insensitive "Modifier " prefix if present
                if candidate.lower().startswith("modifier "):
                    candidate = candidate[9:].strip()  # len("Modifier ") = 9
                # Strip leading hyphen and uppercase
            ...
            # Store the normalized form for modifier fields, original for others
            if is_modifier_field:
    ...
    def _sanitize_extracted_facts(
        ...
        returns `payload`. For every rule in payload["results"]:
          ...
          - target_codes / related_codes are filtered against _CODE_RE:
            * CPT/DRG single codes (3-5 digits, optional alpha suffix: 99284, 469, 99284T)
            * HCPCS single codes (letter + 4 digits: G0378, A0427)
            * ICD-10 single codes (letter + 2+ digits: O30.001, C50.911)
            * Code ranges (99210-99220, A0426-A0428, C50.0-C50.9)
          - required_modifiers / excluded_modifiers are filtered against _MODIFIER_RE:
            * Standard 1-3 alphanumeric chars (25, 59, XU)
            * "Modifier " prefix (Modifier D, Modifier E) — normalized to D, E
            * CMS ambulance notation with leading hyphen (-H, -I, -X) — normalized to H, I, X
            * Case-insensitive (gt/GT both accepted, normalized to GT)
          - revenue_codes are filtered against _REVENUE_CODE_RE:
            * 3-5 digit codes with optional X or digit suffix (0762, 045X, 068X)
            * Revenue code ranges (0450-0459)

        Every drop is logged at WARNING with policy_id + title for traceability.
        Modifier values are normalized ("Modifier " prefix stripped, leading hyphen stripped, uppercased).
    ...
    def _summarize_column_value(
        ...
        if column_type == "badge":
            prompt = textwrap.dedent(f"""
                Summarize payer behavior into a short badge (<=3 words) for column: {column_label}.
            ...
            prompt = textwrap.dedent(f"""
                Summarize payer behavior into ONE concise sentence (<=15 words) for column: {column_label}.
                Payer: {payer_name}
                Rules:
                {sample_text}
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="cell_summarization")
    ...
    def _build_pattern_context(
        ...
        keyword_str = ', '.join(keyword) if keyword else 'N/A'
        return (
    ...
    def _generate_dynamic_columns(
        ...
        prompt = textwrap.dedent(f"""
            You are designing a payer policy summary table for a specific pattern.
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="dynamic_columns")
    ...
    def _generate_elevance_executive_summary(
        ...
        prompt = textwrap.dedent(f"""
            You are writing an executive summary explaining whether an Elevance reimbursement policy plausibly explains a cost anomaly pattern.
        ...
        try:
            response = self._invoke_llm_with_retry(llm, messages, schema="elevance_summary")
    ...
    def prepare_state(
        ...
        **kwargs
        ...
        if context is not None and "pattern" in context:
            ...
            # LLM-classified {dim_name -> role} mapping for the active YAML,
            # forwarded by the orchestrator from the UI's semantic_role_loader.
            # Absence forces the CPT/DRG extractors onto their legacy keyword
            # fallback paths — see TODOs in _extract_*_from_cards.
            raw_semantic_roles = context.get("semantic_roles")
            semantic_roles: Optional[Dict[str, str]] = None
            if isinstance(raw_semantic_roles, dict):
                semantic_roles = {str(k): str(v) for k, v in raw_semantic_roles.items() if k and v}

            # Extract CPT codes from pattern using notebook logic
            cpt_codes = self._extract_cpt_codes_from_pattern(pattern, context, semantic_roles)
            ...
            if not drg_codes:
                ...
                cards = context.get("cards", [])
                if source_card_ids and cards:
                    drg_codes = self._extract_drg_codes_from_cards(
                        source_card_ids, cards, semantic_roles=semantic_roles
                    )
                    if drg_codes:
            ...
            # `llm_mini` routes the CPT path through gpt-5.4-nano; DRG still uses main LLM.
            search_keywords = self._generate_search_keywords(
                drg_codes, cpt_codes, llm=self.llm, user_query=query, llm_mini=self.llm_mini
            )

            # Generate 2-5 title-substring anchors so search_policies_node can
            # drop off-topic policies (those that mention the keyword only in
            # prose). Nano tier — see _generate_title_anchors_from_cpt for the
            # 12-100× reduction rationale. Empty list = skip title regex.
            title_anchors: List[str] = []
            if cpt_codes:
                title_anchors = self._generate_title_anchors_from_cpt(
                    cpt_codes,
                    llm=self.llm_mini or self.llm,
                    user_query=query,
                )

            self.logger.info(
                f"Processing pattern {pattern_rank}: '{self._resolve_pattern_title(pattern)}'"
            )
            self.logger.info(f"Search keywords: {search_keywords}")
            self.logger.info(f"Title anchors: {title_anchors}")
            ...
            # Use CPT codes as search keywords in legacy mode (as a list for consistency).
            # Legacy mode has no LLM-generated title anchors — the caller
            # supplied CPT codes directly, so we skip the title-regex
            # filter and rely on dedup alone in search_policies_node.
        ...
        return {
            ...
            "llm_nano": self.llm_nano,  # GPT-5.4-nano for policy processing
            "llm_mini": self.llm_mini,  # GPT-5.4-mini used only on extract retry
    ...
    def extract_result(self, graph_output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pattern_title = self._resolve_pattern_title(pattern)
        # Subtitle is only available post-format_output; when the pattern
        # itself carries no title, fall back to the rendered table subtitle
        # so the explanation block stays in sync with the summary table.
        if pattern_title == "Unknown Pattern" and subtitle_fallback:
            pattern_title = subtitle_fallback
        ...
        # Detect the insufficient-search-context short-circuit set by
        # search_policies_node so the checks below can produce a
        # calmer message rather than treating a skipped pattern as a failure.
        search_context_status_early = graph_output.get("search_context_status")
        insufficient_context_early = search_context_status_early == "insufficient"
        ...
        elif insufficient_context_early:
            checks.append({"check": "policies_found", "passed": False, "message": "Skipped: insufficient search context"})
        else:
        ...
        if successful > 0:
            ...
            checks.append({"check": "extractions_successful", "passed": False, "message": "Skipped: insufficient search context"})
        else:
        ...
        # Reuse the insufficient-context flag computed alongside the checks
        # above; successful == 0 with insufficient_context is expected, not
        # a failure — treat it as valid_with_warnings.
        insufficient_context = insufficient_context_early
        ...
        if insufficient_context:
            is_valid: Any = "valid_with_warnings"
        elif successful == 0:
            is_valid = False
        ...
        if insufficient_context:
            validation["warnings"].append(
                "Insufficient search context: pattern lacked CPT/HCPCS codes "
                "and no search keywords could be generated. Skipping policy retrieval."
            )
        ...
        if successful == 0 and not insufficient_context:
...
def extract_drg_codes_from_pattern_output(pattern_results: Dict[str, Any]) -> Dict[int, List[str]]:
    ...
    for pattern in patterns:
        ...
        for card_id in source_card_ids:
            ...
            # Method 2: Check source_entity for DRG
            source_entity = card.get('source_entity', {})
            if isinstance(source_entity, dict) and source_entity.get('type') == 'drgs':
                drg_name = source_entity.get('name')
                if drg_name:
...
def extract_lob_state_product_from_pattern_output(pattern_results: Dict[str, Any]) -> Dict[int, Dict[str, List[str]]]:
    ...
    Reads role-tagged values from each source card's ``canonical_dimensions``.
    Role assignments come from the active semantic view via pattern_agent's
    role mapping, so this helper does NOT reference specific semantic field
    names — it stays valid across semantic-view changes.

    - state values ← ``canonical_dimensions['state']``
    - LOB values ← ``canonical_dimensions['line_of_business']``
    - product values ← ``canonical_dimensions['product']``

    ``priority_entities.states`` and ``.products`` are only surfaced when
    also attested by at least one source card, to filter out upstream LLM
    leaks (program names, ZIP codes echoed into ``states``).
    ...
    card_lookup = {card['card_id']: card for card in cards if 'card_id' in card}
    pattern_entities_map: Dict[int, Dict[str, List[str]]] = {}
    ...
    for pattern in patterns:
        ...
        for card_id in source_card_ids:
            ...
            for value in canonical_dims.get('state', []) or []:
                if value:
                    card_states.add(str(value))
            for value in canonical_dims.get('product', []) or []:
                if value:
                    card_products.add(str(value))

        # Intersect priority_entities with card-attested values so leaked
        # non-state values ("BCC STATE SPONSORED PROGRAM", ZIP codes) drop
        # out. Fall back to card-only values when no priority_entities
        # entry survives intersection.
        priority_entities = pattern.get('priority_entities') or {}
        states: set = set()
        for value in priority_entities.get('states') or []:
            if value and str(value) in card_states:
                states.add(str(value))
        if not states:
            states = set(card_states)

        products: set = set()
        for value in priority_entities.get('products') or []:
            if value and str(value) in card_products:
                products.add(str(value))
        if not products:
            products = set(card_products)
