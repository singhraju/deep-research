...
def validate_intent_output(
    ...
    for filter_cond in intent["filters"]:
        ...
        # Skip validation for lob_description since it's loaded directly from database
        if field in field_to_samples and field != "lob_description":
...
def _value_within_range(value: Any, min_val: Any, max_val: Any) -> bool:
    ...
    1. Integer comparison (for YYYYMM or other numeric values)
    2. Date/datetime comparison (for ISO date strings like '2025-01-01')
    3. Lexicographic string comparison (fallback)
    
    Returns True when either bound is missing — the validator should not 
    fabricate a constraint the UI didn't supply.
    """
    if min_val in (None, "") or max_val in (None, ""):
        return True
    
    # Try integer comparison first (YYYYMM, etc.)
    try:
    ...
    # Try date/datetime comparison for ISO date strings
    try:
        ...
        # Check if all three look like ISO dates (YYYY-MM-DD pattern)
        if (len(v_str) >= 10 and v_str[4] == '-' and v_str[7] == '-' and
            len(lo_str) >= 10 and lo_str[4] == '-' and lo_str[7] == '-' and
            len(hi_str) >= 10 and hi_str[4] == '-' and hi_str[7] == '-'):
            v_dt = datetime.fromisoformat(v_str.split()[0])
            lo_dt = datetime.fromisoformat(lo_str.split()[0])
            hi_dt = datetime.fromisoformat(hi_str.split()[0])
            return lo_dt <= v_dt <= hi_dt
