...
class AppConstants:
    ...
    LLM_TIMEOUT: Final[float] = float(os.environ.get("LLM_TIMEOUT", "300.0"))
    LLM_STRUCTURED_TIMEOUT: Final[float] = float(os.environ.get("LLM_STRUCTURED_TIMEOUT", "300.0"))
    ...
    IS_PRODUCTION: Final[bool] = ENV.lower() in ("prod", "production")
    
    # Correlation Subset Table Configuration (Optional)
    CORRELATION_TEMP_TABLE_PREFIX: Final[str] = os.environ.get(
        "CORRELATION_TEMP_TABLE_PREFIX",
        f"DR_{ENV.upper()}_TEMP_"  # Default: DEV_TEMP_, PROD_TEMP_, etc.
    )
    CORRELATION_SUBSET_CLEANUP_ENABLED: Final[bool] = os.environ.get(
        "CORRELATION_SUBSET_CLEANUP_ENABLED", "true"
    ).lower() in ("true", "1", "yes")
    CORRELATION_SUBSET_TTL_HOURS: Final[int] = int(os.environ.get("CORRELATION_SUBSET_TTL_HOURS", "1"))
    
    # Include username in table names/comments (non-production only)
    CORRELATION_INCLUDE_USERNAME: Final[bool] = not IS_PRODUCTION
