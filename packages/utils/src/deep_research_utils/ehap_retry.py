...
def llm_invoke(
    ...
    **invoke_kwargs
    ...
    """
    # Resolve timeout: parameter > explicit in invoke_kwargs > default from AppConstants
    from deep_research_utils.app_constant import AppConstants
    
    if timeout is None and invoke_kwargs.get('timeout') is None:
        timeout = AppConstants.LLM_TIMEOUT
    
    if timeout is not None and invoke_kwargs.get('timeout') is None:
        invoke_kwargs['timeout'] = timeout
        logger.debug(f"LLM invoke timeout: {timeout}s")
...
def structured_llm_invoke(
    ...
    **invoke_kwargs
    ...
    Args:
        ...
        llm_reinitializer: Optional callback to reinitialize LLM with fresh token
        timeout: Request timeout in seconds. If None, uses AppConstants.LLM_STRUCTURED_TIMEOUT (default: 300s)
    ...
    if timeout is None and invoke_kwargs.get('timeout') is None:
        timeout = AppConstants.LLM_STRUCTURED_TIMEOUT
    
    if timeout is not None and invoke_kwargs.get('timeout') is None:
        invoke_kwargs['timeout'] = timeout
        logger.debug(f"LLM structured invoke timeout: {timeout}s")
...
def structured_llm_invoke_with_tokens(
    ...
    **invoke_kwargs
    ...
    if timeout is not None and invoke_kwargs.get('timeout') is None:
        invoke_kwargs['timeout'] = timeout
        logger.debug(f"LLM structured invoke with tokens timeout: {timeout}s")
