...
# Ordered node list + simple layout used by the streaming / notebook views.
_NODE_ORDER = [
    "prepare_payload", "correlation", "pattern",
    "skip_patterns", "reimbursement", "recommendation", "policy", "assemble",
]


def _node_metric(node: str, output: Dict[str, Any]) -> str:
    """One-line, human-readable summary of what a node produced (for progress)."""
    if not isinstance(output, dict):
        return ""
    try:
        if node == "prepare_payload":
            return f"id={output.get('conversation_id', '')}"
        if node == "correlation":
            if output.get("route") == "skip":
                return "unhealthy → skip"
            cr = output.get("correlation_results", {}) or {}
            return f"states={len(cr.get('states', {}))} providers={len(cr.get('providers', {}))} drgs={len(cr.get('drgs', {}))}"
        if node == "pattern":
            if output.get("route") == "skip":
                return "unhealthy → skip"
            pr = output.get("pattern_result", {}) or {}
            n = len((pr.get("output", {}) or {}).get("business_patterns", []))
            return f"business_patterns={n} → {output.get('route', '')}"
        if node == "skip_patterns":
            return "defaults substituted"
        if node == "reimbursement":
            return f"patterns={len(output.get('processed_pattern_result', []))}"
        if node == "recommendation":
            recs = (output.get("recommendation_results", {}) or {}).get("output", {}).get("recommendations", [])
            return f"recommendations={len(recs)}"
        if node == "policy":
            return "ok" if output.get("policy_results") else "empty"
        if node == "assemble":
...
def stream_run(app, state: Dict[str, Any], config: Dict[str, Any], on_event=None) -> Dict[str, Any]:
    """Run one combination with live per-node progress.

    Streams ``app.stream(..., stream_mode='updates')``; for each node emits a
    ``▶ node ✓ (metric)`` line (via ``on_event`` or print) and accumulates the
    updates into the final state, which is returned.
    """
    if on_event is None:
        def on_event(node, metric):
            print(f"  ▶ {node:<16} ✓ {metric}".rstrip())
    ...
    for chunk in app.stream(state, config=config, stream_mode="updates"):
        for node, update in chunk.items():
            if isinstance(update, dict):
                acc.update(update)
            on_event(node, _node_metric(node, update))
    return acc


def stream_demo(scenario: str = "has_patterns", agents: Optional[AgentHelpers] = None) -> Dict[str, Any]:
    """Quick terminal/notebook demo: stream a run with stub agents (no data needed).

    ``scenario`` selects the stub behaviour: ``has_patterns`` (2 patterns),
    ``no_patterns`` (0 patterns) or ``health_skip`` (health check fails).
    Prints the ``▶ node ✓`` progression and returns the final state.
    """
    if agents is None:
        agents = _demo_agent_helpers(scenario)
    app = build_etl_graph(agents=agents)
    state = _demo_state()
    print(f"[stream_demo] scenario={scenario}")
    return stream_run(app, state, {"configurable": {"thread_id": f"demo-{scenario}"}})


def make_studio_graph():
    """Factory used by ``langgraph.json`` for LangGraph Studio / ``langgraph dev``.

    Built with stub agents so the graph loads in Studio without the Snowflake /
    agent network stack. Studio then renders the live graph and lets you run it
    with an ``ETLCombinationState`` input (use ``_demo_state()`` as a template).
    Swap in ``_default_agent_helpers()`` here to drive real agents from Studio.
    """
    return build_etl_graph(agents=_demo_agent_helpers("has_patterns"))


def _demo_state() -> Dict[str, Any]:
    return {
        "env": "pl", "s3_logger": None,
        "snap_year_mnth_nbr": 202604, "trnd_tm_prd_end_mnth_nbr": 202604,
        "trnd_tm_prd_cd": "R3", "lob_shrt_desc": "Commercial_Local_Group",
        "lob_cd": "GBD", "statscl_mdl_cd": "IP AUTH",
        "anomaly_json": {"top_contributors": {"states": [{"name": "CA"}]}},
        "deep_dive_json": {"report": "trend up"}, "insights_lob_code": "GBD",
    }


def _demo_agent_helpers(scenario: str) -> AgentHelpers:
    """Stub helpers that exercise a chosen branch without any network calls."""
    healthy = scenario != "health_skip"
    n_patterns = 2 if scenario == "has_patterns" else 0

    class _Resp:
        status_code = 200

    def pattern(_id, *a, **k):
        bp = [{"pattern_rank": i + 1, "top_pattern": f"Pattern {i+1}", "pattern_details": "d",
               "evidence_summary": ["e"]} for i in range(n_patterns)]
        return _Resp(), {"status": "success", "conversation_id": _id,
                         "output": {"business_patterns": bp, "cards": [], "groups": []}}

    return AgentHelpers(
        generate_agent_common_payload=lambda *a, **k: ({"context": {"filters": []}}, "conv-1"),
        generate_correlation_agent_common_payload=lambda p, *a, **k: p,
        call_correlation_agents=lambda *a, **k: {"states": {"CA": {}}, "providers": {}, "drgs": {}, "procs": {}},
        call_pattern_agents=pattern,
        call_reimbursement_agents=lambda *a, **k: (_Resp(), {"output": {"summary_table": {}, "reimbursement_policies": []}}),
        combine_pattern_reimbursement=lambda *a, **k: {"patterns_data": []},
        call_recommendation_agents=lambda *a, **k: (_Resp(), {"success": True, "output": {"metadata": {}, "recommendations": [{"r": 1}], "skipped_patterns": [], "processing_log": []}, "metadata": {}}),
        call_policy_agent=lambda *a, **k: {"summary_table": {"title": "Payer Policy Summary"}},
        generate_pattern_final_result=lambda s, pr, r: {"rank": pr["pattern_rank"], "pattern_title": pr["top_pattern"]},
        check_api_health=lambda **k: healthy,
        get_semantic_config_path=lambda e, m: "",
    )


# --- Notebook inline animation ------------------------------------------------

_NODE_POS = {
    "prepare_payload": (170, 40), "correlation": (170, 105), "pattern": (170, 170),
    "skip_patterns": (60, 245), "reimbursement": (300, 245), "recommendation": (300, 310),
    "policy": (170, 385), "assemble": (170, 450),
}


def _svg_graph(active: Optional[str] = None, visited=None, skipped: bool = False) -> str:
    """Build a compact inline SVG of the graph, highlighting active/visited nodes."""
    visited = visited or set()
    edges = [
        ("prepare_payload", "correlation"), ("correlation", "pattern"),
        ("pattern", "skip_patterns"), ("pattern", "reimbursement"),
        ("skip_patterns", "policy"), ("reimbursement", "recommendation"),
        ("recommendation", "policy"), ("policy", "assemble"),
    ]
    parts = ['<svg width="380" height="500" viewBox="0 0 380 500" '
             'style="font-family:monospace;background:#f8f8fc;border-radius:12px">']
    for a, b in edges:
        (x1, y1), (x2, y2) = _NODE_POS[a], _NODE_POS[b]
        parts.append(f'<line x1="{x1}" y1="{y1+16}" x2="{x2}" y2="{y2-16}" stroke="#b7b4d0" stroke-width="1.5"/>')
    for node, (x, y) in _NODE_POS.items():
        if node == active:
            fill, stroke, tcol = "#5b4fe0", "#3a2f7a", "#ffffff"
        elif node in visited:
            fill, stroke, tcol = "#cdeedb", "#1f9d63", "#0d4d2d"
        else:
            fill, stroke, tcol = "#eef0ff", "#c9c6e6", "#8a87a3"
        parts.append(
            f'<rect x="{x-72}" y="{y-16}" width="144" height="32" rx="8" fill="{fill}" stroke="{stroke}" stroke-width="2"/>'
            f'<text x="{x}" y="{y+4}" text-anchor="middle" font-size="11" fill="{tcol}">{node}</text>'
        )
    tag = "skipped → END" if skipped else ("done" if "assemble" in visited else "running")
    parts.append(f'<text x="190" y="490" text-anchor="middle" font-size="11" fill="#6f6c86">status: {tag}</text>')
    parts.append("</svg>")
    return "".join(parts)


def display_run_notebook(scenario: str = "has_patterns", app=None, state=None,
                         config=None, delay: float = 0.7):
    """Render a live, inline animated graph in a Jupyter notebook.
    ...
    Pass your own ``app``/``state``/``config`` (from ``build_etl_graph``) to
    animate a real run instead of the stub demo.
    """
    from IPython.display import display, HTML, update_display  # notebook-only

    if app is None:
        app = build_etl_graph(agents=_demo_agent_helpers(scenario))
        state = _demo_state()
    config = config or {"configurable": {"thread_id": f"nb-{scenario}"}}

    disp_id = f"etlgraph-{scenario}"
    visited: set = set()
    display(HTML(_svg_graph(active=None, visited=visited)), display_id=disp_id)
    acc = dict(state)
    skipped = False
    for chunk in app.stream(state, config=config, stream_mode="updates"):
        for node, update in chunk.items():
            if isinstance(update, dict):
                acc.update(update)
                if update.get("route") == "skip" or update.get("status") == "skipped":
                    skipped = True
            update_display(HTML(_svg_graph(active=node, visited=visited, skipped=skipped)), display_id=disp_id)
            visited.add(node)
            time.sleep(delay)
    update_display(HTML(_svg_graph(active=None, visited=visited, skipped=skipped)), display_id=disp_id)
    return acc


def _stream_invoke(app, state: Dict[str, Any], config: Dict[str, Any], label: str = "") -> Dict[str, Any]:
    """Invoke via streaming so the terminal shows live per-node progress."""
    if label:
        print(f"  [stream] {label}")
    return stream_run(app, state, config)
...
def run_pipeline_graph(
    ...
    for idx, combination in enumerate(all_combinations, 1):
        ...
        try:
            ...
            if stream:
                final_state = _stream_invoke(app, initial_state, cfg, label=job_id)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stream", action="store_true",
                        help="Print live per-node progress (▶ node ✓) as each combination runs")
    parser.add_argument("--stream-demo", choices=["has_patterns", "no_patterns", "health_skip"],
                        help="Stream a stubbed demo run (no data/agents) and exit")
    ...
    # Streamed stub demo: no data/agents needed.
    if args.stream_demo:
        stream_demo(args.stream_demo)
        sys.exit(0)

    if not args.env:
        parser.error("--env is required (unless using --show-graph / --stream-demo)")

    try:
        use_snowflake = not args.use_csv
        main(args.env, use_snowflake, args.csv_file, args.lob, args.statscl_mdl_cd,
             args.snap_year_mnth_nbr, args.trnd_tm_prd_cd, args.lob_shrt_desc, args.stream)
    except Exception as exc:
        import traceback
